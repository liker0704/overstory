 ...
 *   Factor:    ...
 *   Priority:  ...
 *   Impact:    +X.X pts  (if estimatedImpact is defined)
 *   Rank:      ...       (if rankReason is defined)
 *   Why now:   ...
 *   Expected:  ...
 *   Action:    ...
 *   Verify:    ...
...
export function renderRecommendation(rec: HealthRecommendation, index: number): void {
	...
	if (rec.estimatedImpact !== undefined) {
		w(`       ${muted("Impact:")}   ${color.green(`+${rec.estimatedImpact.toFixed(1)} pts`)}\n`);
	}
	if (rec.rankReason !== undefined) {
		w(`       ${muted("Rank:")}     ${rec.rankReason}\n`);
	}
