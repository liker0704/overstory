/**
 * Tests for the architecture-review cell definition.
 *
 * Follows the same mock patterns as plan-review.test.ts.
 */
...
function makeHandlerCtx(
	...
	return {
		...
		nodeId: "arch-review:dispatch-critics",
		checkpoint: null,
...
describe("architectureReviewCell.buildSubgraph", () => {
	test("validates via validateGraph with arch-review:dispatch-critics as start", () => {
		const graph = architectureReviewCell.buildSubgraph(defaultConfig);
		const result = validateGraph(graph, { startNodeId: "arch-review:dispatch-critics" });
		expect(result.valid).toBe(true);
	});

	test("all node IDs prefixed with arch-review:", () => {
		const graph = architectureReviewCell.buildSubgraph(defaultConfig);
		for (const node of graph.nodes) {
			expect(node.id.startsWith("arch-review:")).toBe(true);
		}
	});

	test("collect-verdicts node has gate: async and gateTimeout: 600", () => {
		const graph = architectureReviewCell.buildSubgraph(defaultConfig);
		const node = graph.nodes.find((n) => n.id === "arch-review:collect-verdicts");
	...
	test("approved and escalate nodes are terminal", () => {
		const graph = architectureReviewCell.buildSubgraph(defaultConfig);
		const approved = graph.nodes.find((n) => n.id === "arch-review:approved");
		const escalate = graph.nodes.find((n) => n.id === "arch-review:escalate");
...
describe("convergence handler", () => {
	function makeApproveVerdict(): ArchitectureCriticVerdictPayload {
		return {
			criticRole: "structure",
			verdict: "APPROVE",
			concerns: [],
			notes: [],
			round: 1,
	...
	function makeBlockVerdict(concernId: string): ArchitectureCriticVerdictPayload {
		return {
			criticRole: "integration",
			verdict: "BLOCK",
			concerns: [
				{
					id: concernId,
	...
	function makeRecommendVerdict(): ArchitectureCriticVerdictPayload {
		return {
			criticRole: "extensibility",
			verdict: "RECOMMEND_CHANGES",
			concerns: [
				{
					id: "ext-01",
					severity: "medium",
					description: "extensibility issue",
	...
	test("all approve verdicts → approved trigger", async () => {
		...
		const ctx = makeHandlerCtx({
			nodeId: "arch-review:convergence",
	...
	test("BLOCK verdict with no previous concerns → revision-needed", async () => {
		...
		const ctx = makeHandlerCtx({
			nodeId: "arch-review:convergence",
			checkpoint: {
				verdicts: [makeBlockVerdict("arch-01")],
	...
	test("medium severity concern with coupling dimension is elevated to high for stuck detection", async () => {
		...
		const verdict: ArchitectureCriticVerdictPayload = {
			criticRole: "structure",
			verdict: "BLOCK",
			concerns: [
				{
					id: "concern-coupling-01",
					severity: "medium",
					description: "tight coupling issue",
					dimension: "coupling",
				},
			],
			notes: [],
		...
		const ctx = makeHandlerCtx({
			...
			checkpoint: {
				verdicts: [verdict],
				previousBlockConcerns: ["concern-coupling-01"],
				round: 2,
				maxRounds: 3,
			},
		});
		const result = await handler(ctx);
		// medium+coupling(1.0) -> high, so it's in blockingIds, and it matches previousBlockConcerns -> stuck
		expect(result.trigger).toBe("stuck");
	});
});
...
describe("validateVerdictSender", () => {
	test("returns true for known sender", () => {
		const checkpoint = {
			dispatched: true,
			round: 1,
			agents: ["arch-structure-critic", "arch-extensibility-critic"],
		};
		expect(validateVerdictSender("arch-structure-critic", checkpoint)).toBe(true);
	});

	test("returns false for unknown sender", () => {
		const checkpoint = { dispatched: true, round: 1, agents: ["arch-structure-critic"] };
...
// === weightedSeverity tests ===

describe("weightedSeverity", () => {
	test("critical stays critical regardless of dimension", () => {
		expect(weightedSeverity("critical", "abstraction")).toBe("critical");
		expect(weightedSeverity("critical", "coupling")).toBe("critical");
	});

	test("high + coupling (weight 1.0) → critical", () => {
		expect(weightedSeverity("high", "coupling")).toBe("critical");
	});

	test("high + abstraction (weight 0.7) → high", () => {
		expect(weightedSeverity("high", "abstraction")).toBe("high");
	});

	test("medium + coupling (weight 1.0) → high", () => {
		expect(weightedSeverity("medium", "coupling")).toBe("high");
	});

	test("medium + abstraction (weight 0.7) → medium", () => {
		expect(weightedSeverity("medium", "abstraction")).toBe("medium");
	});

	test("low stays low regardless of dimension", () => {
		expect(weightedSeverity("low", "coupling")).toBe("low");
		expect(weightedSeverity("low", "interface-stability")).toBe("low");
	});
});
